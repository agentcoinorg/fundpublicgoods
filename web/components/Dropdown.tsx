  ...
  return (
    <Listbox {...listboxProps}>
      <div className='relative'>
        ...
        {items.length > 0 && (
          <Listbox.Options className='absolute z-10 mt-1 w-full bg-white shadow-lg shadow-primary-shadow/20 max-h-60 rounded-lg py-2 ring-1 ring-white ring-opacity-5 focus:outline-none sm:text-sm'>
            {items.map((item) => (
              <Listbox.Option
                key={item}
                value={item}
